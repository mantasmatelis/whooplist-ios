...
-(void)selectPP:(id)sender {
    if (APP_DEL.mainViews[@"Main"]) {
        UIActionSheet *as = [[UIActionSheet alloc] initWithTitle:@"Profile Picture" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Take Photo", @"Choose from Library", nil];
        [as showInView:((UIViewController*)APP_DEL.mainViews[@"Main"]).view];
    }
}
...
-(void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex > 1)
        return;
    _ppPicker.sourceType = (buttonIndex == 0) ? UIImagePickerControllerSourceTypeCamera : UIImagePickerControllerSourceTypePhotoLibrary;
    if (APP_DEL.mainViews[@"Main"])
